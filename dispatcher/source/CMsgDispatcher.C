...
void CMsgDispatcher::procMsg(TUniNetMsg* msg){
	printf("dispatcher get msg from %d\n", msg->oAddr.logAddr);
	if(msg->oAddr.logAddr == (UINT)LOGADDR_SIP_PSA){
		...
		errorLog("###CMsgDispatcher: received an unexpected message from logaddr %d,This task will be ended.\n",
				msg->oAddr.logAddr);
		end();
	}
...
void CMsgDispatcher::handleMsgFromSipPSA(TUniNetMsg* msg){
	...
	case SIP_INVITE:
		...
		if(m_pDialogCtrl->getDialogAddr(generateSipUniqID(msg), tAddr)){
			printf("CMsgDispatcher::handleMsgFromSipPSA this is a reinvite from xlite\n");
...
//		storeUniqID(msg, SIP_TYPE);
	...
	case SIP_RESPONSE:
	{
		...
		if(m_pDialogCtrl->getDialogAddr(uniqID, tAddr)){
			...
			printf("handleMsgFromSipPSAcan not get addr by %s", uniqID);
			errorLog("handleMsgFromSipPSAcan not get addr by %s", uniqID);
		}
		break;
	}
	default:
		printf("received unknown msgName %s\n", msg->getMsgName());
		break;
	}
}
...
//			printf("CMsgDispatcher getSoredUniqID: %s\n", getStoredUniqID());
		...
		if(!m_pDialogCtrl->storeDialog(uniqID, msg->oAddr)){
			printf("handleMsgFromSipCall can not store %s, logdaddr: %d", uniqID.c_str(), msg->oAddr.logAddr);
			errorLog("handleMsgFromSipCall can not store %s, logdaddr: %d", uniqID.c_str(), msg->oAddr.logAddr);
		}
	}
